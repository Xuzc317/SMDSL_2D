...
class VlmParser:
    ...
    def generate_instruction_examples(
        ...
        """
        根据场景语义画像生成 N 个情境化的操作指令示例，用于 Tab 2 预设按钮。

        Args:
            scene_profile: Spatial-RAG 生成的场景画像 dict。
            file_format:   文件格式（json/svg/png），影响可用语义丰富度。
            n_examples:    生成示例数。

        Returns:
            List[{"label": str, "instruction": str, "context": JSON str}]
        """
        if not _OPENAI_AVAILABLE:
            raise VlmParserError("openai 包未安装，无法调用 LLM。")

        room_type = scene_profile.get("room_type", "未知")
        objects_raw = scene_profile.get("objects", []) or []
        objects_list = (
            [o.get("label", str(o)) if isinstance(o, dict) else str(o)
             for o in objects_raw[:10]]
        )
        constraints_hint = scene_profile.get("recommended_constraints", "")
        spatial_summary = scene_profile.get("spatial_summary", "")

        sys_prompt = (
            "你是机器人任务指令设计师。基于给定的场景语义画像，"
            f"生成 {n_examples} 个**具体的、可执行的**机器人自然语言指令示例。\n\n"
            "每个示例必须包含：\n"
            "  - label: 示例简称（格式：'① 动词+宾语（场景,约束类型）'，≤16字）\n"
            "  - instruction: 自然语言指令（30~60字，含避障/时序/姿态等约束，但不含坐标数值）\n"
            "  - nearest_objects: 该指令涉及到的场景实体名称列表（直接从画像中取，≤6个）\n\n"
            "严格输出 JSON 格式，无多余字段：\n"
            '{"examples": [{"label": "...", "instruction": "...", "nearest_objects": [...]}]}\n\n'
            "硬性约束：\n"
            "① 指令类型要多样：至少覆盖【操作+避障】【限时操作】【巡检/无抓取】三类之一\n"
            "② nearest_objects 只包含场景中真实存在的物体\n"
            "③ 不能出现坐标、像素等数值\n"
            "④ 严禁用『先...再...后...』命令式时序，应用『在 T 秒内...，全程保持...』声明式"
        )
        ...
        profile_summary = {
            ...
            "spatial_summary": spatial_summary[:200] if spatial_summary else "",
            "recommended_constraints": constraints_hint,
        }
        user_prompt = (
            f"场景信息：\n{json.dumps(profile_summary, ensure_ascii=False, indent=2)}\n\n"
            f"请生成 {n_examples} 个指令示例，严格按 JSON 格式输出："
        )
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                for i, ex in enumerate(examples_raw[:n_examples], start=1):
                    label = ex.get("label") or f"示例 {i}"
                    instruction = ex.get("instruction", "")
                    nearest = ex.get("nearest_objects", [])
                    ctx = json.dumps(
                ...
                if results:
                    return results
                raise VlmParserError("LLM 返回 examples 为空")
            except _OPENAI_RETRIABLE as e:
                last_err = e
            except VlmParserError:
                raise
            except Exception as e:
                raise VlmParserError(f"generate_instruction_examples 异常: {e}") from e
            if attempt < self.max_retries:
                time.sleep(self.retry_backoff_s * attempt)

        raise VlmParserError(
            f"generate_instruction_examples 失败（重试 {self.max_retries} 次）。"
            f"最后错误: {type(last_err).__name__}: {last_err}"
        )
...
def generate_instruction_examples(
    ...
    *,
    ...
    """
    根据场景语义画像生成 N 个情境化的操作指令示例。

    Args:
        scene_profile: Spatial-RAG 生成的场景画像 dict（含 room_type, objects 等）。
        file_format:   输入文件格式（json / svg / png 等），用于调整示例风格。
        n_examples:    生成示例数量，默认 3。

    Returns:
        List of {"label": str, "instruction": str, "context": JSON str}
    """
    parser = VlmParser(model_name=model_name, api_key=api_key, max_retries=2)
    return parser.generate_instruction_examples(
