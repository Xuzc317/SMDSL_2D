...
def render_pipeline_quad(
    ...
    # (a) 二值栅格 — 把门洞拎出来用淡蓝叠加
    grid_rgb = np.stack([grid, grid, grid], axis=-1).astype(np.float32)
    ax_grid.imshow(grid_rgb, origin="upper")
    ...
    # (c) A* 访问热力 — 用 99 分位裁剪 + sqrt 压缩，对比度比 log1p 强很多
    heat = np.sqrt(visit_count.astype(np.float64))
    if heat.max() > 0:
        vmax = np.percentile(heat[heat > 0], 99)
    else:
        vmax = 1.0
    im2 = ax_visit.imshow(
        heat, cmap="magma", origin="upper", vmin=0, vmax=max(1.0, vmax),
    )
    n_reached = int((visit_count > 0).sum())
    n_free = int((grid == 1).sum())
    ax_visit.set_title(
        f"(c) A* 漫水访问热力  (reached={n_reached}/{n_free} free, "
        f"{n_reached / max(1, n_free):.1%})"
    )
    plt.colorbar(im2, ax=ax_visit, fraction=0.046, pad=0.04,
                 label="√visits (99 分位裁剪)")
...
def render_source_cad(
    ...
    if boundary:
        ...
        if coords:
            ...
            ax.fill(xs, ys, color="#f5f5f0", edgecolor="#222",
    ...
    legend = [
        Patch(facecolor="#f5f5f0", edgecolor="#222", label="boundary"),
...
def render_clickable_topology(
    ...
    cmap = ListedColormap([
        "#dddddd",  # 0 unknown
        "#dddddd",  # 1 loading (treat as unknown)
        "#7fd17f",  # 2 path (亮绿)
        "#000000",  # 3 obstacle
        "#ff9a3c",  # 4 inflated
    ])
    ...
    from matplotlib.patches import Patch  # noqa: PLC0415
    legend = [
        Patch(facecolor="#7fd17f", edgecolor="#3a9a3a", linewidth=1.2,
              label="🟢 可走区 — 可在此点击选起/终点"),
        Patch(facecolor="#ff9a3c", edgecolor="#b06010", linewidth=1.0,
              label="🟠 膨胀缓冲 — 机器人重心禁入区"),
        Patch(facecolor="#1a1a1a", edgecolor="#ffffff", linewidth=0.5,
              label="⬛ 墙 / 障碍物"),
        Patch(facecolor="#cccccc", edgecolor="#888", linewidth=0.5,
              label="⬜ 未到达区（孤立空间）"),
    ]
