...
def _extract_dwg_semantics(entities: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    summary = {"lines": 0, "polylines": 0, "circles": 0, "arcs": 0, "texts": 0, "blocks": 0}
...
def dispatch_cad(
    ...
    if ext in RASTER_EXTS:
        ...
        return result
    if ext in OSM_EXTS:
        # OSM 模式：直接多边形填充，绕过 rasterize_to_grid + A* flood-fill
        from cad_parser.osm_ingestion import parse_osm_to_topology  # noqa: PLC0415
        return parse_osm_to_topology(
            str(p),
            resolution=resolution,
            padding_m=padding_m,
            wall_thickness_m=wall_thickness_m,
